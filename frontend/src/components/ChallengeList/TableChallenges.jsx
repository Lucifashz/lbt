...
export default function TableChallenges() {
   ...
   const getChallengesByChallenged = async(userId) => {
      await axios.get(`http://localhost:3000/challenges-by-challenged-id/${userId}`)
      .then((response) => {
   ...
   const getChallengesByChallenger = async(userId) => {
      await axios.get(`http://localhost:3000/challenges-by-challenger-id/${userId}`)
      .then((response) => {
   ...
   const updateChallenge = async (id, status, referee, type, date, challengerId, challengerPartner, challengedId, challengedPartner) => {
      if (type === "single") {
         ...
         await axios.post("http://localhost:3000/matches", single)
            .then((response) => {  })
            .catch((error) => {  })
      } else if (type === "double") {
         const double = {
         ...
         await axios.post("http://localhost:3000/matches", double)
            .then((response) => {  })
            .catch((error) => {  })
      } else {
         console.log(status);
      ...
      await axios.patch(`http://localhost:3000/challenges-match-status/${id}`, {
         "match-status": status,
